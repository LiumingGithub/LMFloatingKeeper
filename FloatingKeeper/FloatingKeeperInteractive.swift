...
open class FloatingKeeperInteractive: BaseGestureInteractive {
    ...
    struct Constant {
        static let buttonSize = CGSize(width: 150, height: 150)
        static let minShowingPercent: CGFloat = 0.2
    ...
    // MARK: -
    // invoke when touchesInsideDrawer value changed
    open func didChangeInsideState() -> Void {
        wantsKeepedBtn.isSelected = touchesInsideButton
        if #available(iOS 10.0, *) {
            if touchesInsideButton,
                let feedback = feedback as? UIImpactFeedbackGenerator {
