...
open class BaseGestureInteractive: UIPercentDrivenInteractiveTransition {
    ...
    let draggingEdge: InteractiveDraggingEdge
...
//        print("location: \(location) percent:\(percent)")
        return transform(percent)
    }
    
    // 默认的 percent 计算根据拖动的距离和屏幕的长度的比例
    // 如果觉得 percent 的变化比较缓慢，或者希望非线形变化。可以乘以一个常数。或者使用幂次及开方处理
    // 默认为不做变化
    open func transform(_ percent: CGFloat) -> CGFloat {
        return percent
    }
